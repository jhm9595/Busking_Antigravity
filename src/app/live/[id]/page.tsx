...
export default function AudienceLivePage() {
    ...
    return (
        <div className="bg-background text-foreground h-[100dvh] flex flex-col w-full md:max-w-xl mx-auto font-display overflow-hidden selection:bg-primary/30">
            <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-border px-4 py-3 flex items-center justify-between shadow-2xl shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <Link href={`/singer/${singer?.id}`} className="flex items-center gap-3 group">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-foreground/60 font-bold uppercase tracking-widest italic">{realtimeStatus === 'connected' ? t('live.status_live') : t('live.status_syncing')}</span>
                        </div>
                    </Link>
                </div>
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 shadow-lg italic ${isFollowed ? 'bg-foreground/5 text-foreground/60 border border-border' : 'bg-primary text-primary-foreground shadow-primary/30'}`}
                    >
                        {isFollowed ? t('common.following') : t('common.follow')}
                    </button>
                    <button onClick={handleShare} className="p-2.5 rounded-xl bg-foreground/5 border border-border hover:bg-foreground/10 text-foreground/60 transition-all active:scale-95"><Share2 className="w-4 h-4" /></button>
                </div>
            </header>
            ...
            <main className="flex-1 overflow-y-auto flex flex-col p-4 pb-32 custom-scrollbar">
                {isCompleted ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center py-12 animate-in fade-in zoom-in duration-500 italic">
                        ...
                        <p className="text-foreground/60 mb-10 max-w-[240px] leading-relaxed">{t('live.ended_desc')}</p>
                        <Link href={`/singer/${singer?.id}`} className="w-full max-w-xs bg-primary py-4 rounded-2xl font-black shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all text-sm uppercase tracking-widest text-primary-foreground">{t('live.view_singer_profile')}</Link>
                    </div>
                ) : (
                    <div className="flex-1 flex flex-col space-y-4">
                        <section className="bg-card/50 rounded-[32px] p-6 border border-border shadow-xl">
                            <div className="flex items-center justify-between mb-5">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setShowRequestModal(true)} className="bg-primary/20 hover:bg-primary/40 text-primary px-3 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all border border-primary/30 italic flex items-center gap-2 shadow-lg"><Music className="w-3 h-3" /> {t('song_request.title')}</button>
                                    <span className="text-[10px] font-black bg-foreground/5 px-2.5 py-1 rounded-full text-foreground/60 uppercase tracking-widest italic">{performance.songs?.length || 0} {t('live.tracks')}</span>
                                </div>
                            </div>
                            <div className={`space-y-3 ${isChatOpen ? 'max-h-[300px] overflow-y-auto custom-scrollbar pr-1' : ''}`}>
                                {performance.songs?.length > 0 ? (
                                    performance.songs.map((s: any, i: number) => {
                                        ...
                                        return (
                                            <div key={i} className={`group p-4 rounded-2xl border transition-all duration-500 ${s.status === 'completed' ? 'bg-foreground/10 border-border text-foreground/60' : 'bg-card border-border shadow-lg shadow-foreground/10 hover:border-primary/30'}`}>
                                                <div className="flex justify-between items-center">
                                                    <div className="min-w-0">
                                                        <p className="font-black text-base flex items-center gap-3 truncate text-foreground uppercase italic tracking-tight group-hover:text-primary transition-colors">{s.title}{s.status === 'completed' && <Check className="w-4 h-4 text-emerald-500" />}</p>
                                                        <p className="text-[10px] font-bold text-foreground/60 uppercase tracking-[0.2em] mt-0.5">{s.artist}</p>
                                                    </div>
                                    ...
                                    <div className="p-10 text-center text-xs text-foreground/50 bg-foreground/10 border border-dashed border-border rounded-[24px] italic font-bold">{t('performance.details.empty_setlist')}</div>
                                )}
                            </div>
                        </section>
                        ...
                        {isChatOpen ? (
                            <section className="bg-card rounded-[32px] border border-border flex flex-col flex-1 min-h-[450px] overflow-hidden shadow-2xl relative mb-20 animate-in slide-in-from-bottom-6 duration-700">
                                <div className="p-4 bg-background/60 backdrop-blur-md border-b border-border flex justify-between items-center sticky top-0 z-10">
                                    ...
                                    <div className="flex items-center gap-2 bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20">
                                        ...
                                        <span className="text-[10px] font-black text-primary uppercase tracking-widest italic">{viewingCount} {t('live.watching')}</span>
                                    </div>
                                </div>
                                <ChatBox
                                    performanceId={id}
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-12 bg-foreground/5 rounded-[48px] border border-dashed border-border text-center mt-6 min-h-[300px] italic shadow-inner">
                                <div className="w-20 h-20 bg-card rounded-full flex items-center justify-center mb-6 opacity-30 shadow-2xl"><MessageSquareOff className="w-10 h-10 text-foreground" /></div>
                                <p className="text-foreground/60 text-xs font-black uppercase tracking-[0.3em]">{t('chat.closed_placeholder')}</p>
            ...
            {showRedirectionModal && (
                <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-3xl flex flex-col items-center justify-center p-10 text-center animate-in fade-in duration-1000 italic">
                    ...
                    <p className="text-foreground/60 mb-14 max-w-[280px] leading-relaxed">{t('live.ended_desc')}</p>
                    <div className="bg-foreground/5 border border-border px-12 py-8 rounded-[40px] mb-14 relative shadow-2xl">
                        <span className="text-6xl font-mono font-black text-transparent bg-clip-text bg-gradient-to-b from-primary to-primary/80">{redirectCountdown}</span>
                        <p className="text-[10px] font-black text-foreground/60 uppercase tracking-[0.2em] mt-3">{t('live.redirecting')}</p>
                    </div>
                    <div className="flex flex-col gap-4 w-full max-w-[280px]">
                        <button onClick={() => setShowRedirectionModal(false)} className="bg-foreground/5 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-foreground/10 transition-all border border-border text-foreground">{t('live.stay_here')}</button>
                        <Link href={`/singer/${singer?.id}`} className="bg-primary py-5 rounded-2xl font-black text-xs uppercase tracking-widest text-center shadow-xl shadow-primary/30 hover:bg-primary/90 transition-all border border-primary/30 text-primary-foreground">{t('live.view_singer_profile')}</Link>
                    </div>
                </div>
            )}
